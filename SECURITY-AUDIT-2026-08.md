...
| 编号 | 问题 | 等级 |
| --- | --- | --- |
| H-1 | 会话期间持续全屏镜像，而非按需取帧 | 高（隐私过度采集） |
| M-1 | ML Kit 遥测 / Firebase 安装 ID 默认开启，与文档陈述不符 | 中 |
| M-2 | 屏幕文字写入全局剪贴板，未标记敏感、不清除 | 中 |
| M-3 | 无 Gradle Wrapper、无依赖锁定与校验，构建不可复现 | 中（供应链） |
| L-1 | 屏幕文字经系统广播跨进程传递（同进程组件之间） | 低 |
| L-2 | 结果面板无 `FLAG_SECURE`，按钮无遮挡点击防护 | 低 |
| L-3 | 已签名 APK 提交进仓库，release 无 `signingConfig` | 低（发布链） |
| L-4 | `allowBackup="false"` 未覆盖 Android 12+ 设备迁移 | 低 |
| I-1 | 传递依赖版本偏旧且完全未显式声明 | 信息 |
| I-2 | 自定义权限被抢注会导致安装失败 | 信息 |
| I-3 | 自身悬浮面板可能被下一帧 OCR 到 | 信息 |
